...
found:
  ...
  */
  ...
  // PA4: kalloc() and proc_pagetable() (via walk) may now sleep, because a
  // failed allocation triggers swap_out(), which does disk I/O. Holding
  // p->lock across a sleep makes sched() panic("sched locks"). The slot is
  // already reserved (state==USED), so on a single CPU no one else will
  // touch it -- drop p->lock during the allocations and retake it after.
  // AI was used (Claude) to diagnose and fix this.
  release(&p->lock);
...
kfork(void)
{
  ...
  // PA4: uvmcopy()/mmap_copy() may now sleep -- if a parent page is swapped
  // out, uvmcopy() calls swap_in(), which does disk I/O (bread/virtio) and
  // blocks. allocproc() returned np with np->lock held; holding a spinlock
  // across a sleep makes sched() see noff>1 and panic("sched locks").
  // np is not yet RUNNABLE and not yet linked to any parent, so no other
  // context can touch it -- safe to drop np->lock for the copy and retake
  // it afterwards. AI was used (Claude) to diagnose and fix this.
  release(&np->lock);
