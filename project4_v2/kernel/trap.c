...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    // PA4: scause 12 is an instruction-page fault. User text/rodata pages
    // carry PTE_U and are tracked in the LRU, so they can be swapped out
    // too. If we only handle load(13)/store(15) faults, an instruction
    // fetch from a swapped-out page falls through to vmfault(), which
    // zero-fills it and destroys the code. Treat 12 like 13/15 so the
    // swap-in path runs. AI was used (Claude) to diagnose and fix this.
    uint64 va = r_stval();
