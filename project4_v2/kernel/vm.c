...
vmfault(pagetable_t pagetable, uint64 va, int read)
{
  ...
  // PA4: the page may not be unmapped -- it may be swapped out (PTE_V==0,
  // PTE_S==1). walkaddr()/ismapped() only test PTE_V, so copyin/copyout
  // and the trap fallback would otherwise treat a swapped page as a fresh
  // lazy page and zero-fill it, destroying its contents. Detect PTE_S and
  // bring the real page back from swap instead. AI was used (Claude).
  {
    pte_t *pte = walk(pagetable, va, 0);
    if(pte != 0 && (*pte & PTE_V) == 0 && (*pte & PTE_S)){
      if(swap_in(pagetable, va) < 0)
        return 0;
      return walkaddr(pagetable, va);   // now resident -> return its pa
    }
  }
