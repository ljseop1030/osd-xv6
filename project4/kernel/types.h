...
/* AI assisted - proj4
    ...
    First, we proceeded as instructed by the README, but it eventually caused error
    so we made it to resolve it by changing the field type to
    uint64 *pagetable; , which is identical to pagetable_t.
*/
// proj4 README - requires 
struct page {
    struct page *next, *prev;
    uint64 *pagetable; // error caused with pagetable_t  pagetable;  
    uint64       vaddr;
};